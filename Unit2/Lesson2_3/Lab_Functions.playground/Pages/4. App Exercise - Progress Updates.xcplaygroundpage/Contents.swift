...
func progressUpdate (steps : Int , goal : Int){
    if (steps < goal * 1/10) {
        print("You're of to a good start.")
    }
    else if (steps < goal * 1/2) {
...
progressUpdate(steps: 250, goal: 1000)
progressUpdate(steps: 5000, goal: 10000)
progressUpdate(steps: 90000, goal: 100000)

//:  Your fitness tracking app is going to help runners stay on pace to reach their goals. Write a function called pacing that takes four `Double` parameters called `currentDistance`, `totalDistance`, `currentTime`, and `goalTime`. Your function should calculate whether or not the user is on pace to hit or beat `goalTime`. If yes, print "Keep it up!", otherwise print "You've got to push it just a bit harder!"
func pacing (currentDistance : Double, totalDistance : Double, currentTime: Double, goalTime: Double) {
    if ((currentDistance + currentTime) < ((totalDistance + goalTime) * 1/2)) {
...
pacing(currentDistance: 675, totalDistance: 1000, currentTime: 35, goalTime: 60)
